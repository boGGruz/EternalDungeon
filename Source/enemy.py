...
class Enemy:
    ...
    def spawn_enemy(self, percent_hp):
        new_enemy = Enemy(percent_hp)
        self.enemies.append(new_enemy)


    def spawn_demon(self, percent_hp):
        new_demon = Demon(percent_hp)
            ...
            for demon in self.demons:
                for arrow in bullet.arrows:
                    if demon.rect.colliderect(arrow.rect):
                        self.sound_shoot_enemy.play()
                        bullet.arrows.remove(arrow)
                        demon.demon_hp -= 3 * self.percent_damage
                        demon.move = False
                        self.last_move = pg.time.get_ticks()
                        if demon.demon_hp <= 0:
        ...
        if player.player_hp > 1:
            for enemy in self.enemies:
                ...
                if enemy.move:
                    ...
                    enemy.add_bullet()
            for devil in self.demons:
                devil.handle_animation_demon(player)
                devil.demon_moving(player, dt)
                devil.demon_hit_player(player, character)
...
class Demon:
    def __init__(self, percent_hp):
        ...
        self.y = random.randint(240, 660)
        self.textures = Textures_enemy()
        self.rect = pg.Rect(self.x, self.y, 48, 48)
        self.demon = self.textures.demon["devil_1"]
        self.speed = 240
        self.last_update_frame = pg.time.get_ticks()
        self.index = 1
        self.shadow = pg.Surface((200, 100), pg.SRCALPHA)
        pg.draw.ellipse(self.shadow, (0, 0, 0, 160), (0, 0, 30, 15))
        self.spawned = False
        self.demon_hp = 7 * percent_hp
        self.move = True
        self.last_frame = pg.time.get_ticks()
        self.sound = Textures_Hud_Sound()
        self.sound_bite = self.sound.sound["bite_sound"]
        self.sound_bite.set_volume(0.7)
        self.demon_spawned = False
        self.last_update_time = 0


    def handle_animation_demon(self, player):
        current_time = pg.time.get_ticks()
        if current_time - self.last_update_time >= 62.5:
            if self.demon_spawned == False:
                self.spawn_demons()
                self.last_update_time = current_time
            else:
                if current_time - self.last_update_frame > 70:
                    self.demon = self.textures.demon[f"devil_{self.index}"]
                    self.demon = pg.transform.scale(self.demon, (48, 48))
                    if self.rect.centerx > player.rect.centerx:
                        self.demon = pg.transform.flip(self.demon, True, False)
                    self.index += 1
                    if self.index == 9:
                        self.index = 1
                    self.last_update_frame = pg.time.get_ticks()

    def demon_moving(self, player, dt):
        if self.demon_spawned == True:
            angle = math.atan2(self.rect.y - player.rect.y - 10, self.rect.x - player.rect.x - 28)
            self.rect.x -= self.speed * dt * math.cos(angle)
            self.rect.y -= self.speed * dt * math.sin(angle)

    def spawn_demons(self):
        self.demon = self.textures.demon[f"devil_spawn_{self.index}"]
        self.demon = pg.transform.scale(self.demon, (48, 48))
        if self.index == 5:
            self.demon_spawned = True
            self.index = 0
        self.index += 1

    def draw_hp(self, surface, player):
        surface.blit(self.shadow, (self.rect.x + 8, self.rect.y + 40))
        surface.blit(self.demon,
                     (self.rect))
        health_bar_width = 10
        health_bar_height = 4
        health_bar_x = self.rect.x - health_bar_width
        health_bar_y = self.rect.y
        if player.player_hp > 1:
            pg.draw.rect(surface, (255, 0, 0), (health_bar_x, health_bar_y, health_bar_width * 2.3, health_bar_height))
            remaining_health_bar_width = health_bar_width * (self.demon_hp / 3)
            pg.draw.rect(surface, (0, 255, 0),
                         (health_bar_x, health_bar_y, remaining_health_bar_width, health_bar_height))

    def demon_hit_player(self, player, character):
        current_time = pg.time.get_ticks()
        if current_time - self.last_frame > 1000:
