                ...
                for enemy in enemys.enemies:
                    if enemy.rect.colliderect(self.sword_rect):
                        ...
                        if enemy.enemy_hp <= 0:
                            ...
                            hud.score += 40
                        self.last_time = pg.time.get_ticks()
            if current_time - self.last_frame > 350:
                for demon in enemys.demons:
                    if demon.rect.colliderect(self.sword_rect):
                        demon.demon_hp -= 2 * enemys.percent_damage
                        enemys.sound_shoot_enemy.play()
                        if demon.demon_hp <= 0:
                            enemys.demons.remove(demon)
                            hud.score += 45
