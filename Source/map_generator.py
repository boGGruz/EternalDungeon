...
class Map_generator:
    def __init__(self, width, height, window, percent_hp):
        ...
        self.back_rooms = {
            0: self.index}
        self.lobby = Map(window, width, height, self.back_rooms[self.index], 10, 0, 0)
        self.rooms = {
            "room_0": self.lobby}
        ...
        self.key_rand_room = random.randint(2, 5)
        self.hole_spawn = False
        self.restart = False
        self.key_up = False
        self.hole_room = 0
    ...
    def generate_world(self, window, character_rect, player, enemy, prev_rect):
        if f"room_{self.index}" not in self.rooms:
            if self.index > 2:
                self.rand = random.randint(1, 15)
                self.key_rand_room = random.randint(2, 3)
                if self.index > 4 and self.hole_spawn == False:
                    self.key_rand_room = 2
                if self.hole_spawn == False:
                    self.rooms[f"room_{self.index}"] = Map(window, self.ROOM_WIDTH, self.ROOM_HEIGHT,
                                                           self.back_rooms[self.index], self.rand, self.count_room,
                                                           self.key_rand_room)
                elif self.hole_spawn == True:
                    self.rooms[f"room_{self.index}"] = Map(window, self.ROOM_WIDTH, self.ROOM_HEIGHT,
                                                           self.back_rooms[self.index], self.rand, self.count_room, 3)
                if self.key_rand_room == 2:
                    self.hole_spawn = True
                if self.rand > 4:
                    self.count_room += 1
            else:
                self.rooms[f"room_{self.index}"] = Map(window, self.ROOM_WIDTH, self.ROOM_HEIGHT,
                                                       self.back_rooms[self.index], 5, self.count_room, 0)
        ...
        self.lock_room(window, player, prev_rect)
        self.rooms[f"room_{self.index}"].stage_clear(enemy, self.index, self.percent_hp)
        if self.index == 0:
    ...
    def cleared_room(self, enemy):
        if not enemy.enemies and not enemy.demons:
            if not self.clear:
                self.rooms[f"room_{self.index}"].clear = True
                self.clear_room += 1
                self.clear = True
        else:
            self.clear = False

    def lock_room(self, window, player, prev_rect):
        if self.rooms[f"room_{self.index}"].clear == False:
            if self.index != 0 or self.rooms[f"room_{self.index}"].bonus > 4 and self.rooms[
                f"room_{self.index}"].bonus < 10:
    ...
    def bonus_room(self, window, character, player, enemy):
        ...
        elif self.rooms[f"room_{self.index}"].bonus == 1 and self.rooms[f"room_{self.index}"].item_pick_up == False:
            self.items.healing_potion(window, character)
        ...
        if self.rooms[f"room_{self.index}"].key_rand_room == 2 and self.rooms[
            f"room_{self.index}"].item_pick_up == False and self.rooms[f"room_{self.index}"].clear == True:
            self.items.hole_triger(window, character)
            self.hole_room = self.index
        if self.rooms[f"room_{self.index}"].count_room == 5 and self.rooms[
            f"room_{self.index}"].item_pick_up == False and self.rooms[f"room_{self.index}"].clear == True:
            self.items.key(window, character)

    def pick_up(self, character, player, enemy):
        keys = pg.key.get_pressed()
        if self.rooms[f"room_{self.index}"].bonus == 1 and character.colliderect(self.items.potion_rect) and keys[
            pg.K_f] and self.rooms[f"room_{self.index}"].get_pressed == False:
            player.heal += 1
            self.rooms[f"room_{self.index}"].get_pressed = True
            return True
        elif self.rooms[f"room_{self.index}"].bonus <= 4 and character.colliderect(self.items.attack_rect) and keys[
            pg.K_f] and self.rooms[f"room_{self.index}"].get_pressed == False:
            enemy.percent_damage += 0.25
            self.rooms[f"room_{self.index}"].get_pressed = True
            return True
        elif character.colliderect(self.items.hole_rect) and keys[
            pg.K_f] and self.index == self.hole_room and self.key_up == True:
            self.restart = True
        elif character.colliderect(self.items.key_rect) and keys[pg.K_f] and self.rooms[
            f"room_{self.index}"].get_pressed == False:
            self.key_up = True
            self.rooms[f"room_{self.index}"].get_pressed = True
            return True
