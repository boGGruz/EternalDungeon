...
class Hud:
    def __init__(self):
        ...
        self.font1 = pg.font.Font("ThaleahFat.ttf", 40)
        self.score = 0
        self.record = f.readline()
        f.seek(0)
        self.dada = 2
    ...
    def score_bar(self, window):
        ...
        if int(score) > int(self.record):
            self.record = score
            with open('record.txt', 'w') as g:
                g.truncate()
                g.write(str(self.record))
        text_surface_1 = self.font.render(score, True, (255, 255, 255))
        text_surface_2 = self.font1.render("Record: " + self.record, True, (255, 255, 255))
        window.blit(text_surface_1, (1100, 8))
        window.blit(text_surface_2, (1150, 71))