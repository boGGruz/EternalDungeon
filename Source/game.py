...
class Game:
    def __init__(self):
        ...
        self.percent_hp = 1.0
        self.enemy = Enemy(self.percent_hp)  # Enemy
        ...
        self.generate_world = Map_generator(self.width, self.height, self.window, self.percent_hp)
            ...
            if event.type == pg.QUIT:
                ...
                if event.key == pg.K_ESCAPE and self.play_menu == False:
                    if self.player.player_dead == True:
                        ...
                        self.weapon = Weapon()
                        self.enemy = Enemy(self.percent_hp)
                        self.generate_world = Map_generator(self.width, self.height, self.window, self.percent_hp)
                    else:
    ...
    def update(self):  # Все обновляется по частоте FPS: 150 в секунду
        ...
        if self.generate_world.restart == True:
            self.percent_hp += 0.5
            self.player.rect.x = self.width / 2 - 37
            self.player.rect.y  = self.height / 2 - 37
            self.generate_world = Map_generator(self.width, self.height, self.window, self.percent_hp)
    ...
    def interface(self):
        ...
        damage = self.font.render('damage: ' + str(self.enemy.percent_damage) + 'x', True,
                                  pg.Color('white'))
        self.window.blit(fps_text, (10, 10))
        self.window.blit(heal_user, (self.width - 200, self.height - 100))
        self.window.blit(damage, (self.width - 200, self.height - 150))
