...
class Map:
    def __init__(self, window, width, height, back_room, bonus, count_room, key_rand_room):
        self.window = window
        ...
        self.count_demon_current = random.randint(1, 3)
        self.count_enemy = 0
        self.count_demon = 0
        ...
        for row in self.tileData:
            for col in row:
                if col == "U":  # Стена фон
                    ...
                    pf = pg.transform.rotate(pf, 90)
                    self.window.blit(pf, (self.i, self.j))
                elif col == "c1":  # Угол краёв карты вне зоны игрока
                    pf = self.textures.texture_room["border_corner"]
                    pf = pg.transform.rotate(pf, 180)
                    pf = pg.transform.scale(pf, (self.PLATFORM_WIDTH, self.PLATFORM_HEIGHT))
                    self.window.blit(pf, (self.i, self.j))
                elif col == "c2":  # Угол краёв карты вне зоны игрока
                    pf = self.textures.texture_room["border_corner"]
                    pf = pg.transform.rotate(pf, -90)
                    pf = pg.transform.scale(pf, (self.PLATFORM_WIDTH, self.PLATFORM_HEIGHT))
                    self.window.blit(pf, (self.i, self.j))
                elif col == "c3":  # Угол краёв карты вне зоны игрока
                    pf = self.textures.texture_room["border_corner"]
                    pf = pg.transform.scale(pf, (self.PLATFORM_WIDTH, self.PLATFORM_HEIGHT))
                    self.window.blit(pf, (self.i, self.j))
                elif col == "c4":  # Угол краёв карты вне зоны игрока
                    pf = self.textures.texture_room["border_corner"]
                    pf = pg.transform.rotate(pf, 90)
                    pf = pg.transform.scale(pf, (self.PLATFORM_WIDTH, self.PLATFORM_HEIGHT))
                    self.window.blit(pf, (self.i, self.j))
    ...
    def stage_clear(self, enemy, index, percent_hp):
        if index != 0 and self.clear == False and self.count_enemy <= self.count_enemy_current:
            ...
            if current_time - self.last_tick > 300 and self.bonus > 4:
                enemy.spawn_enemy(percent_hp)
                self.count_enemy += 1
                self.last_tick = pg.time.get_ticks()
        elif index != 0 and self.clear == False and self.count_enemy > self.count_enemy_current:
            current_time = pg.time.get_ticks()
            if current_time - self.last_tick > 300 and self.bonus > 4:
                if self.count_demon <= self.count_demon_current:
                    self.count_demon += 1
                    enemy.spawn_demon(percent_hp)
                    self.last_tick = pg.time.get_ticks()
